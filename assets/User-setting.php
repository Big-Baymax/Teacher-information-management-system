...
<div id="wrapper">
			...
			<ul class="nav" id="main-menu">
				...
				<li>
					...
					<ul class="nav nav-second-level">
						...
						<li>
							<a href="User-setting.php" class="active-menu waves-dark">系统用户管理</a>
						</li>
						<li>
	...
	<div id="page-wrapper">
		...
		<div id="page-inner">
					...
					<div class="modal-dialog" role="document">
						<div class="modal-content">
							...
							<div class="modal-body">
								<div ><label>用户名：</label>	<input type="text" name="uesrname" id="uesrname" class="form-control" value="" /></div>
								<div ><label>密码：</label>	<input type="text" name="password" id="password" class="form-control" value="" /></div>
								<div ><input name="group1" type="checkbox" id="test" /><label for="test">是否为管理员</label></div>
							</div>
			...
			<div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel">
				<div class="modal-dialog" role="document">
					<div class="modal-content">
						...
						<div class="modal-body">
							<div ><label>用户名：</label>	<input type="text" name="edit-uesrname" id="edit-uesrname" class="form-control"value="" /></div>
							<div ><label>密码：</label>	<input type="text" name="edit-password" id="edit-password" class="form-control" value=""/></div>
							<div ><input name="group1" type="checkbox" id="edit-test" /><label for="edit-test">是否为管理员</label></div>
						</div>
						<div class="modal-footer">
							<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
			...
			<!--/. 网页脚部  -->
			<footer>
				<p>Copyright &copy; 2017 - 2020 宁德师范学院计算机系</p>
	...
	$.ajax({
		...
		success: function(r) {
			...
			for (var i = 0; i < result.length; i++) {
				...
				if(localStorage.a == 0){
					s += '<tr style="display: none"><td>' + result[i].id + '</td><td>' + result[i].teacherid + '</td><td>' + result[i].password + '</td><td>' + result[i].teachername + '</td>'
							+ '<td>' + result[i].identity + '</td>' +
					...
					s += '<tr><td>' + result[i].id + '</td><td>' + result[i].teacherid + '</td><td>' + result[i].password + '</td><td>' + result[i].teachername + '</td>'
							+ '<td>' + result[i].identity + '</td>' +
						'<td><button class="btn btn-info " type="button" id="delete">删除</button></td>' + '<td><button class="btn btn-info " type="button" data-toggle="modal" name="edit" id="edit" data-target="#editModal">修改</button></td></tr>';
			}
				$('tbody').html(s);
				...
				if(result.result == 1){
					...
					for (var i = 1 ; i< $('tr').length;i++) {
						...
						if( x.text()==localStorage.id ){
							$('tr').eq(i).find(' td:eq(3)').text($('#edit-uesrname').val());
							$('tr').eq(i).find(' td:eq(2)').text($('#edit-password').val());
							$('tr').eq(i).find(' td:eq(4)').text(power);
...
<script type="text/javascript">
	...
	$(document).on('click','button[name =edit]',function(){
		var power = $(this).parent().parent().children().eq(4).text();
		if(power == '是'){
		...
		document.getElementById("edit-uesrname").value = $(this).parent().parent().children().eq(3).text();
			...
			$.ajax({
				...
				success: function (r) {
					...
					if (result != null) {
						var s = '';
						for (var i = 0; i < result.length; i++)
							s += '<tr><td>' + result[i].id + '</td><td>' + result[i].teachername + '</td><td>' + result[i].password + '</td>'
									+ '<td>' + result[i].test + '</td>' +
									'<td><button class="btn btn-info " type="button" id="delete">删除</button></td>' + '<td><button class="btn btn-info " type="button" data-toggle="modal" name="edit" id="edit" data-target="#editModal">修改</button></td></tr>';
						$('tbody').html(s);
					}
		...
		$.ajax({
			...
			success: function(r) {
				...
				if(result.result == 1){
					...
					s += '<tr><td>' + z+ '</td><td>' + $("#uesrname").val() + '</td><td>' + $("#password").val() + '</td>'
							+ '<td>' + power+ '</td>'+
							'<td><button class="btn btn-info" type="button" id="delete">删除</button></td>'+'<td><button class="btn btn-info " type="button" data-toggle="modal" name="edit" id="edit" data-target="#editModal">修改</button></td></tr>';
					$('tbody').append(s);
					location.reload();
