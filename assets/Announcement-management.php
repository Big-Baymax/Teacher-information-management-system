...
<div id="wrapper">
	...
	<nav class="navbar-default navbar-side" role="navigation">
		<div class="sidebar-collapse">
			<ul class="nav" id="main-menu">
				...
				<!--                    </li>-->
				<li>
					<a href="javascript:void(0)" id="System-setting" class="waves-effect waves-dark" data-toggle="dropdown"><i class="fa fa-edit"></i> 系统管理<span class="fa arrow"></span> </a>
					<ul class="nav nav-second-level" >
						...
						<li>
							<a href="Announcement-management.php" class="active-menu waves-dark">公告发布管理</a>
						</li>
						<li>
	...
	<div id="page-wrapper">
		...
		<div id="page-inner">
			<div >
				<div class="col-sm-3 ">
					<button class="btn btn-info add" type="button" data-toggle="modal" data-target="#myModal">添加公告</button>
				</div>
				<!--/. 添加信息模态框  -->
					...
					<div class="modal-dialog" role="document">
						<div class="modal-content">
							...
							<div class="modal-body">
								<div ><label>公告标题：</label>	<input type="text" name="title" id="title" class="form-control" /></div>
								<div ><label>公告内容：</label>	<textarea id="noticetext" rows="" cols="" class="form-control"></textarea></div>
							</div>
			...
			<div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel">
				<div class="modal-dialog" role="document">
					<div class="modal-content">
						...
						<div class="modal-body">
							<div ><label>公告标题：</label>	<input type="text" name="edit-title" id="edit-title" class="form-control" /></div>
							<div ><label>公告内容：</label>	<textarea name="edit-noticetext" id="edit-noticetext" rows="" cols="" class="form-control"></textarea></div>

						</div>
						<div class="modal-footer">
							<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
							<button id="edit-btn" type="button" class="btn btn-primary" data-dismiss="modal">确定</button>
						</div>
		...
		success: function(r) {
			...
			if(result.result == 0)
			{
				...
				for (var i = 0; i < result.length; i++)
					if(localStorage.a==0) {
						s += '<tr><td>' + result[i].id + '</td><td>' + result[i].title + '</td><td>' + result[i].noticetext + '</td>'
								+ '<td>' + result[i].author + '</td><td>' + result[i].noticetime + '</td>' +
	...
	$(document).on('click','#edit-btn',function(){
		...
		$.ajax({
			...
			data: {
				...
				noticetext:$('#edit-noticetext').val()
			},
			success:function(r){
				var result = $.parseJSON(r);
				if(result.result == 1){
					...
					for (var i = 1 ; i< $('tr').length;i++) {
						...
						if( x.text()==localStorage.id ){
							$('tr').eq(i).find(' td:eq(1)').text($('#edit-title').val());
							$('tr').eq(i).find(' td:eq(2)').text($('#edit-noticetext').val());
...
<script type="text/javascript">
	...
	$(document).on('click','button[name =edit]',function(){
		...
		document.getElementById("edit-title").value = $(this).parent().parent().children().eq(1).text();;
		document.getElementById("edit-noticetext").value = $(this).parent().parent().children().eq(2).text();
...
//
//						s += '<tr><td>' + z+ '</td><td>' + $("#title").val() + '</td><td>' + $("#noticetext").val() + '</td>'
//						+ '<td>' + $("#author").val()+ '</td><td>' + $("#noticetime").val() +  '</td>'+
//						'<td><button class="btn btn-info " type="button" id="delete">删除</button></td>'+'<td><button class="btn btn-info " type="button" data-toggle="modal" name="edit" id="edit" data-target="#editModal">修改</button></td></tr>';
